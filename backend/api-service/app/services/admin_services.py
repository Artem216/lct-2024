...
async def get_all_users(current_user_id: int) -> List[AllUser]:
    ...
    Эта функция получает список всех пользователей из базы данных, исключая текущего пользователя и пользователей, отмеченных как удаленные.
    Она также подсчитывает количество положительных и отрицательных отзывов, сгенерированных каждым пользователем.

    Args:
        current_user_id (int): Идентификатор текущего пользователя.

    Returns:
        List[AllUser]: Список объектов AllUser, представляющих всех пользователей.
    """
...
async def raise_user(user_id :int) -> UserDto:
    ...
    Эта функция обновляет статус пользователя до администратора в базе данных.

    Args:
        user_id (int): Идентификатор пользователя, которого нужно повысить.

    Returns:
        UserDto: Обновленная информация о пользователе, включая его идентификатор, email, статус администратора и имя.
    """
    db = await get_connection() 
