...
async def get_cards_by_theme(theme: str) -> List[TopCards]:
    """
    Получить карточки по заданной теме.

    Эта функция получает список карточек из базы данных на основе указанной темы (продукта).
    Карточки включают в себя URL изображения, рейтинг, имя пользователя и другие связанные данные.

    Args:
        theme (str): Тема (продукт), по которой нужно получить карточки.

    Returns:
        List[TopCards]: Список объектов TopCards, представляющих карточки с заданной темой.
    """
    db = await get_connection()
