...
def agregate_users(df, num_tags):
    ...
    df.app_vehicle_ind = df.app_vehicle_ind.astype('float')
    df.cnt_tr_all_3m = df.cnt_tr_all_3m.astype('float')
    df.cnt_tr_buy_3m = df.cnt_tr_buy_3m.astype('float')
    df.cnt_tr_mobile_3m = df.cnt_tr_mobile_3m.astype('float')
    df.cnt_tr_oil_3m = df.cnt_tr_oil_3m.astype('float')
    df.sum_zp_12m = df.sum_zp_12m.astype('float')
    
    if len(df) > 1: 
       
        age = df.age.mean()
        
        if np.all(df['gender'].isna()):
            gender = np.nan
        else:
            gender = df.gender.mode()[0]
            
        if np.all(df['app_vehicle_ind'].isna()):
            car = np.nan
        else:
...
def prompt_dataset_pipeline(file, num_tags = 3): # Можно регулировать количество тегов
